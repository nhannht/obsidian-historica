/**
 * WikiWars-ZH accuracy test for HistoricaChrono ZH locale.
 * Corpus: 1224 annotated date expressions from 22 Chinese Wikipedia war articles.
 * Same structure as ChronoParser.test.ts (WikiWars EN) and ChronoParserJAWiki.test.ts.
 */
...
const FIXTURE_FILE = join(__dirname, "fixtures/wikiwars_zh_curated.json");
...
describe("WikiWars-ZH — HistoricaChrono ZH locale", () => {
	...
	it("accuracy report", async () => {
		...
		console.log(`\n=== WikiWars-ZH Accuracy (zhCustomChrono) ===`);
		...
		expect(totalHits / totalCount).toBeGreaterThan(0.3);
	});
});
