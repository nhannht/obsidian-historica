	...
	return (
		<svg width={width} height={totalH} style={{display: "block", overflow: "visible"}}>
			...
			<line x1={0} y1={baselineY} x2={width} y2={baselineY}
				stroke="var(--int-border)" strokeWidth={0.75}/>
			{years.map((y, i) => (
				<line key={i}
					x1={toX(y)} y1={2} x2={toX(y)} y2={tickH - 1}
					stroke="var(--int-primary)" strokeWidth={1.5} opacity={0.7}
				/>
			))}
			<text x={1} y={totalH} style={{fontSize: "8px", fill: "var(--int-on-surface-faint)", fontFamily: "var(--int-font-mono)"}}>{min}</text>
			<text x={width - 1} y={totalH} textAnchor="end" style={{fontSize: "8px", fill: "var(--int-on-surface-faint)", fontFamily: "var(--int-font-mono)"}}>{max}</text>
		</svg>
	);
}
